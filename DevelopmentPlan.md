...
- [x] ~~Order placement na may totoong pera~~ ✅ **VERIFIED (2026-07-12)** — `tests/live_e2e_order.py`: totoong BUY 6.25 shares @ 0.48 → FILLED → SELL @ 0.46 → FILLED sa live CLOB ($0.28 spread lang ang gastos). **Mga kritikal na nadiskubre sa proseso:** (a) CLOB V2 migration (2026-04-28) — kailangang `py-clob-client-v2`, tinatanggihan na ang lumang client ("invalid order version"); (b) mga account na ginawa pagkatapos ng Apr 2026 ay DEPOSIT WALLET accounts — signature_type=3 at ang Funder ay ang "Address (For API use only)" sa polymarket.com Settings (HINDI ang username address); (c) kailangan ng KAHIT ISANG manual UI trade para ma-deploy ang deposit wallet + approvals bago gumana ang API orders
